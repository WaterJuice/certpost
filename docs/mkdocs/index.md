...
Managing SSL certificates across multiple services is tedious — requesting certs, setting DNS records, renewing before expiry, distributing updated certs. certpost automates the entire lifecycle: it issues Let's Encrypt certificates using DNS-01 challenges via the Namecheap API, renews them automatically, and provides an API for other services to fetch their certificates.

## Features

- **Web admin panel** — manage subdomains, API tokens, and configuration from your browser
- **Automatic issuance** — ACME v2 with DNS-01 challenges via the Namecheap API
...
1. Go to the **Configuration** tab and enter your Namecheap API credentials, base domain, and ACME email
